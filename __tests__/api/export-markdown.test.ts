...
function createRequest(params: Record<string, string> = {}) {
  const url = new URL("https://clarusapp.io/api/export/markdown")
...
describe("GET /api/export/markdown", () => {
  ...
  it("returns 403 when free-tier user tries to export", async () => {
    ...
    // Re-mock authenticateRequest with new supabase that returns free tier
  ...
  it("returns 200 with text/markdown content-type on success", async () => {
    const request = createRequest({ id: VALID_UUID })
    const response = await GET(request)
  ...
  it("returns 200 and markdown body that includes the content title", async () => {
    const request = createRequest({ id: VALID_UUID })
    const response = await GET(request)
  ...
  it("returns 200 even when summary is null (analysis pending)", async () => {
    ...
    expect(response.status).toBe(200)
    const text = await response.text()
    expect(text).toContain("Analysis pending")
  })
