...
export async function GET(request: Request) {
  try {
    ...
    const safeTitle = (content.title || "report")
      ...
      .substring(0, 50)
    const filename = `truth-check-${safeTitle}.pdf`
...
async function generatePDF(content: Record<string, unknown>, summary: Record<string, unknown> | null): Promise<ArrayBuffer> {
  ...
  // === TRUTH CHECK (Most Important) ===
  const truthCheck = summary.truth_check as TruthCheckData | null
  if (truthCheck) {
    y += 3
    addText("TRUTH CHECK", { size: 14, color: COLORS.primary, bold: true })
    y += 2
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `Generated by Truth Checker (infosecops.io) | Page ${i} of ${totalPages}`,
      pageWidth / 2,
      pageHeight - 10,
      { align: "center" }
    )
