  ...
  return (
    <div className={cn("flex gap-2", message.role === "user" ? "justify-end" : "justify-start")}>
      {message.role === "assistant" && (
        <div className="flex-shrink-0 w-6 h-6 rounded-full bg-brand flex items-center justify-center shadow-lg shadow-blue-500/20">
      ...
      <div
        className={cn(
          ...
          message.role === "user"
            ? "bg-brand text-white shadow-lg shadow-blue-500/20 px-3 py-2"
      ...
      {message.role === "user" && (
        <div className="flex-shrink-0 w-6 h-6 rounded-full bg-brand flex items-center justify-center shadow-lg">
    ...
    onError: (error) => {
      ...
      if (msg.includes("limit reached") || msg.includes("Upgrade")) {
        toast.error("Chat limit reached", {
          description: "Upgrade your plan for more chat messages.",
      ...
      <div className="px-3 py-2 flex justify-between items-center border-b border-white/[0.08] bg-white/[0.02]">
        ...
        {threadId && messages.length > 0 && (
          <button
            ...
            className="h-6 px-2 text-[0.625rem] text-gray-400 hover:text-white hover:bg-white/[0.08] rounded-md transition-colors flex items-center gap-1"
          >
          ...
          {isLoading && messages[messages.length - 1]?.role === "user" && (
            <div className="flex gap-2 justify-start">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-white/[0.04] border border-white/[0.08]">
                ...
                <span className="text-[0.625rem] text-gray-400">Thinking...</span>
              </div>
            </div>
          )}
        ...
        <div className="flex-grow relative">
          ...
          {isListening && !localInput && (
            <div className="absolute left-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
              ...
              <span className="text-[0.625rem] text-red-400 font-medium">Listening...</span>
            </div>
          )}
          {isListening && localInput && (
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
              <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" />
              <span className="text-[0.625rem] text-red-400 font-medium">Rec</span>
            </div>
          )}
        </div>
        <Button
